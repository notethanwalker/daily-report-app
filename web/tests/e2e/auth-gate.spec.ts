...
test("unauthenticated users can switch between sign-in and registration",async({page})=>{
  ...
  const modes=page.getByRole("group",{name:"Account access mode"});
  const signInMode=modes.getByRole("button",{name:"Sign in"});
  const createMode=modes.getByRole("button",{name:"Create account"});
