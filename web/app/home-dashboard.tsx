...
export default function HomeDashboard({market,rotation,onOpenMarkets,onOpenMacro}:{market:Record<string,Market>;rotation:Rotation|null;onOpenMarkets:()=>void;onOpenMacro:()=>void}){
 ...
 return <div className="dashboard-layout-zone" aria-label="Customizable report dashboard">
  <div className="card home-dashboard-card reveal-card" data-dashboard-card="market_dashboard"><div className="section-head"><div><span className="eyebrow">Market overview</span><h2>Market Dashboard</h2><p className="muted">Latest visible market input: {newestAsOf(rows)}</p></div><button className="text-btn" onClick={onOpenMarkets}>All markets →</button></div>
  ...
  <div className={styles.sentimentCluster} data-dashboard-card="sentiment">
