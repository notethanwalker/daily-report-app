...
export default function GroupedNavigation({nav,active,commandActive,access,onCommand}:{nav:Element;active:string;commandActive:boolean;access:AccessState|null;onCommand:()=>void}){
 ...
 function select(name:string,e?:React.MouseEvent){
   ...
   if(name==="Command Center"){onCommand();return}
   // Route every normal-tab selection through the augmentation bridge. This is
   // required when leaving Command Center because that bridge owns commandActive;
   // clicking the hidden legacy button directly changes the base tab but otherwise
   // leaves Command Center mounted over it.
   window.dispatchEvent(new CustomEvent("daily-report-nav-request",{detail:name}));
 }
