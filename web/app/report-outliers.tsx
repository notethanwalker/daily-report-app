...
export default function ReportOutliers({items}:{items:Outlier[]}){
 ...
 return <div className="card reveal-card report-outliers-card"><div className="section-head"><div><span className="eyebrow">Largest absolute moves</span><h2>Notable Outliers</h2></div><div className="outlier-sort"><span className="muted">Sort</span>{(["1d","7d","30d"] as SortKey[]).map(k=><button key={k} className={`toggle ${sort===k?"active":""}`} onClick={()=>setSort(k)}>{k.toUpperCase()}</button>)}</div></div><div className="outlier-table"><div className="outlier-table-head"><span>Ticker</span><span>1D</span><span>7D</span><span>30D</span><span>Rel Vol</span></div>{rows.map(o=><div className="outlier-table-row" key={o.symbol}><div><strong>{o.symbol}</strong><small>{o.reason}</small></div><span className={(o.change_percent??0)>=0?"positive":"negative"}>{pct(o.change_percent)}</span><span className={(o.seven_day_percent??0)>=0?"positive":"negative"}>{pct(o.seven_day_percent)}</span><span className={(o.thirty_day_percent??0)>=0?"positive":"negative"}>{pct(o.thirty_day_percent)}</span><span>{o.relative_volume==null?"—":`${o.relative_volume.toFixed(2)}x`}</span></div>)}</div></div>;
}
