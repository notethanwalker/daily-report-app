...
function DecisionStack({account}:{account:Account}){
 ...
 return <main className="v4-shell">
  <header className="v4-header"><div><span className="v4-kicker">DAILY REPORT V4 · DEVELOPMENT BUILD</span><h1>Decision Stack</h1><p>Research → Macro → Opportunity → Deployment</p></div><div className="v4-user"><span>{account.name}</span><small>{overview?.version||"4.2-dev"}</small></div></header>
  ...
  {active==="macro"&&<section className="v4-grid">
   ...
   <article className="v4-card"><span className="label">Persisted history</span><h2>{layer?.rotation_history_days??0} days</h2><p className="muted">Canonical daily state observations accumulating for later forward-outcome calibration.</p></article>
  ...
  {active==="opportunity"&&<section className="v4-grid">
   <article className="v4-card wide"><div className="section-head"><div><span className="label">Candidate funnel</span><h2>Broad market → qualified setups</h2></div><strong>{funnel?.candidates?.length??layer?.candidate_count??0}</strong></div><div className="funnel-stages">{(funnel?.stages||[]).map((x:any,i:number)=><div key={x.name}><span>{i+1}</span><strong>{x.name}</strong><b>{x.input} → {x.output}</b><small>{x.rule}</small></div>)}</div>{funnel?.deep_enrichment_jobs_added>0&&<p className="muted">Queued {funnel.deep_enrichment_jobs_added} bounded fundamentals-enrichment jobs for scanner-only finalists.</p>}</article>
