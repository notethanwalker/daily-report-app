...
async function requestJson(url:string,init?:RequestInit){const r=await fetch(url,{cache:"no-store",credentials:"include",...init});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d?.detail||`HTTP ${r.status}`);return d}
...
function DecisionStack({account}:{account:Account}){
 ...
 const[loading,setLoading]=useState(true),[error,setError]=useState(""),[deployLoading,setDeployLoading]=useState(false),[researchLoading,setResearchLoading]=useState(false),[enrichLoading,setEnrichLoading]=useState(false),[enrichMsg,setEnrichMsg]=useState("");
 ...
 async function loadRotation(){if(rotation)return;try{setRotation(await requestJson(`${API}/api/v1/stack/rotation`))}catch(e:any){setError(e?.message||String(e))}}
 async function loadFunnel(force=false){if(funnel&&!force)return;try{setFunnel(await requestJson(`${API}/api/v1/stack/candidates?limit=60`))}catch(e:any){setError(e?.message||String(e))}}
 ...
 async function loadResearch(){const s=symbol.trim().toUpperCase();if(!s)return;setResearchLoading(true);setError("");try{setResearch(await requestJson(`${API}/api/v1/stack/research/${encodeURIComponent(s)}`));setSymbol(s)}catch(e:any){setError(e?.message||String(e))}finally{setResearchLoading(false)}}
 useEffect(()=>{load()},[]);
 ...
 return <main className="v4-shell">
  <header className="v4-header"><div><span className="v4-kicker">DAILY REPORT V4 · DEVELOPMENT BUILD</span><h1>Decision Stack</h1><p>Research → Macro → Opportunity → Deployment</p></div><div className="v4-user"><span>{account.name}</span><small>{overview?.version||"4.4-dev"}</small></div></header>
  ...
  {active==="research"&&<section className="v4-grid">
   ...
   <article className="v4-card wide research-search"><div><span className="label">Security workspace</span><h2>Unified ticker research</h2><p className="muted">Cache-first composition of market, fundamentals, theme/sector rotation, flow, theses and score history.</p></div><div className="control-row"><input value={symbol} onChange={e=>setSymbol(e.target.value.toUpperCase())} onKeyDown={e=>{if(e.key==="Enter")loadResearch()}} placeholder="Ticker"/><button onClick={loadResearch} disabled={researchLoading}>{researchLoading?"Loading…":"Open"}</button></div></article>
   {research&&<>
  ...
  {active==="macro"&&<section className="v4-grid">
   <article className="v4-card wide"><div className="section-head"><div><span className="label">Rotation model v4</span><h2>Leadership + transition state</h2></div><strong>{rotation?.rows?.length??0} groups</strong></div><div className="v4-table rotation-table">{(rotation?.leaders||[]).map((x:any)=><div key={x.symbol}><strong>{x.symbol}</strong><span>{x.name}</span><span>{words(x.state)}</span><span>{x.stale_input?"stale":x.transition_ready?`${n(x.delta_3_observations,2)} Δ`:`${x.observations||0} obs`}</span><b>{n(x.rotation_pressure??x.rotation_score,2)}</b></div>)}</div></article>
   <article className="v4-card"><span className="label">Early rotation</span><h2>Improving laggards</h2>{(rotation?.early_rotation||[]).map((x:any)=><div className="compact-row" key={x.symbol}><strong>{x.symbol}</strong><span>{x.name}</span><b>{n(x.conviction,0)}</b></div>)}</article>
   <article className="v4-card"><span className="label">Outflow risk</span><h2>Weakening leadership</h2>{(rotation?.outflow_risk||[]).map((x:any)=><div className="compact-row" key={x.symbol}><strong>{x.symbol}</strong><span>{x.name}</span><b>{n(x.rotation_pressure,1)}</b></div>)}</article>
  ...
  {active==="opportunity"&&<section className="v4-grid">
   <article className="v4-card wide"><div className="section-head"><div><span className="label">Candidate funnel</span><h2>Broad market → qualified setups</h2></div><div className="control-row"><strong>{funnel?.candidates?.length??0}</strong><button onClick={enrichFunnel} disabled={enrichLoading||!(funnel?.deep_enrichment_symbols||[]).length}>{enrichLoading?"Queueing…":"Enrich shortlist"}</button></div></div><div className="funnel-stages">{(funnel?.stages||[]).map((x:any,i:number)=><div key={x.name}><span>{i+1}</span><strong>{x.name}</strong><b>{x.input} → {x.output}</b><small>{x.rule}</small></div>)}</div>{enrichMsg&&<p className="muted">{enrichMsg}</p>}</article>
   <article className="v4-card wide"><h2>Highest-ranked market candidates</h2><div className="v4-table opportunities funnel-table">{(funnel?.candidates||[]).slice(0,30).map((x:any)=><div key={x.symbol}><strong>{x.symbol}</strong><span>{x.rotation_proxy||x.sector||"Unclassified"}</span><span>{words(x.setup_type||x.bucket||x.rotation_state||"tracked")}</span><span>100MA {x.price_vs_ma100_percent==null&&x.ma100_distance==null?"—":`${n(x.price_vs_ma100_percent??x.ma100_distance,1)}%`}</span><span>W%R {n(x.williams_r_14??x.williams_feature,1)}</span><span>{x.enrichment_status||words(x.rotation_state)||"—"}</span><b>{n(x.funnel_score??x.score,1)}</b></div>)}</div><p className="muted">{funnel?.methodology||layer?.methodology}</p></article>
  </section>}
