...
export default function OpportunityFormulaBuilderV4({onOpen}:{onOpen:(symbol:string)=>void}){
 ...
 const[loading,setLoading]=useState(true),[error,setError]=useState(""),[addKey,setAddKey]=useState(""),[addFilterKey,setAddFilterKey]=useState(""),[saveName,setSaveName]=useState(""),[expanded,setExpanded]=useState<string|null>(null),[activePresetId,setActivePresetId]=useState<string|number|null>("default");
 ...
 function markCustom(){if(activePresetId!==null)setActivePresetId(null)}
 ...
 function changeWeight(key:string,value:number){if(!Number.isFinite(value)||value<=0)return;markCustom();setCriteria(c=>({...c,[key]:value}))}
 function remove(key:string){if(activeKeys.length<=1)return;markCustom();const next={...criteria};delete next[key];setCriteria(next)}
 function add(){if(!addKey)return;markCustom();setCriteria(c=>({...c,[addKey]:10}));setAddKey("")}
 function addFilter(){const info=filterInfo(addFilterKey);if(!addFilterKey||!info)return;markCustom();setFilters(xs=>[...xs,{field:addFilterKey,operator:info.default_operator||info.operators?.[0]||"<=",value:Number(info.default_value??0)}]);setAddFilterKey("")}
 function updateFilter(i:number,patch:Partial<HardFilter>){markCustom();setFilters(xs=>xs.map((x,j)=>j===i?{...x,...patch}:x))}
 function removeFilter(i:number){markCustom();setFilters(xs=>xs.filter((_,j)=>j!==i))}
 function usePreset(p:any){const next={...(p.criteria||DEFAULT)},nextFilters=[...(p.filters||[])];setActivePresetId(p.id);setSaveName(p.built_in?"":p.name);setCriteria(next);setFilters(nextFilters);setExpanded(null);run(next,nextFilters,sortKey,sortDir)}
 async function save(){const name=saveName.trim();if(!name)return;setError("");try{const created=await api("/api/v1/opportunities/formulas",{method:"POST",body:JSON.stringify({name,criteria,filters})});setActivePresetId(created.id);await loadPresets()}catch(e:any){setError(e?.message||String(e))}}
 async function updatePreset(){if(!activePreset||activePreset.built_in)return;const name=saveName.trim()||activePreset.name;setError("");try{const updated=await api(`/api/v1/opportunities/formulas/${activePreset.id}`,{method:"PUT",body:JSON.stringify({name,criteria,filters})});setSaveName(updated.name);setActivePresetId(updated.id);await loadPresets()}catch(e:any){setError(e?.message||String(e))}}
 async function duplicatePreset(){const base=(saveName.trim()||activePreset?.name||"Opportunity Formula").replace(/\s+Copy$/i,"");setError("");try{const created=await api("/api/v1/opportunities/formulas",{method:"POST",body:JSON.stringify({name:`${base} Copy`,criteria,filters})});setSaveName(created.name);setActivePresetId(created.id);await loadPresets()}catch(e:any){setError(e?.message||String(e))}}
 async function del(p:any){if(p.built_in)return;setError("");try{await api(`/api/v1/opportunities/formulas/${p.id}`,{method:"DELETE"});if(String(activePresetId)===String(p.id)){setActivePresetId("default");setSaveName("")}await loadPresets()}catch(e:any){setError(e?.message||String(e))}}

 return <div className="formula-builder-v4">
  <div className="formula-toolbar">
   <div><span className="label">Active Opportunity formula</span><h2>{activePreset?.name||"Custom Opportunity Formula"}</h2><p className="muted">{meta?.formula?.label||"Weights are normalized automatically."} Index scores rank relative setup quality; they are not return probabilities.</p></div>
