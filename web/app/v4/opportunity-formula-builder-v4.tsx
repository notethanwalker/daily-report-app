...
export default function OpportunityFormulaBuilderV4({onOpen}:{onOpen:(symbol:string)=>void}){
 ...
 function usePreset(p:any){const next={...(p.criteria||DEFAULT)},nextFilters=[...(p.filters||[])];setCriteria(next);setFilters(nextFilters);setExpanded(null);run(next,nextFilters,sortKey,sortDir)}
