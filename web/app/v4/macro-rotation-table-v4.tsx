...
export default function MacroRotationTableV4({rotation,onOpen}:{rotation:any;onOpen:(symbol:string)=>void}){
 ...
 return <>
  ...
  <div className="table-controls" role="group" aria-label="Macro rotation table controls"><label>View<select value={group} onChange={e=>setGroup(e.target.value)}><option value="all">All groups</option><option value="leaders">Current leaders</option><option value="early">Early rotation</option><option value="outflow">Outflow risk</option></select></label><label>Rank metric<select value={metric} onChange={e=>chooseMetric(e.target.value as Metric)}>{(Object.keys(LABELS) as Metric[]).map(key=><option key={key} value={key}>{LABELS[key]}</option>)}</select></label><button type="button" onClick={()=>setDirection(x=>x==="asc"?"desc":"asc")}>{direction==="asc"?"↑ Asc":"↓ Desc"}</button><span>{rows.length} shown</span></div>
