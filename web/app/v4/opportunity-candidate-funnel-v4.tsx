...
export default function OpportunityCandidateFunnelV4({onOpen,formula}:{onOpen:(symbol:string)=>void;formula:ActiveOpportunityFormula|null}){
 ...
 return <section className="formula-funnel-v4">
  ...
  {data&&<>
   ...
   <div className="funnel-candidate-list">{(data.candidates||[]).map((c:any)=>{const cc=c.candidate_context||{},sections=cc.sections||{},ev=cc.evidence||{};return <details key={c.symbol} className={`funnel-candidate attention-${c.attention_stage||"watch"}`}><summary><b>#{c.priority_rank} {c.symbol}</b><span>{stage(c.attention_stage)}</span><span>Formula #{c.formula_rank} · {n(c.formula_score)}</span><span>Priority {n(c.contextual_priority_score)}</span><span>{stage(c.convergence?.state)}</span></summary><div className="funnel-candidate-detail"><div className="source-state-grid"><span><i>Williams</i><b>{n(c.williams_r_14,2)}</b></span><span><i>100MA distance</i><b>{c.price_vs_ma100_percent==null?"—":`${n(c.price_vs_ma100_percent,2)}%`}</b></span><span><i>Rotation</i><b>{stage(c.rotation_state)}</b></span><span><i>Feature context</i><b>{stage(c.feature_context_status)}</b></span><span><i>Fundamentals</i><b>{freshness(sections.fundamentals)}</b></span><span><i>News</i><b>{freshness(sections.news)} · {cc.news?.count??0}</b></span><span><i>News evidence</i><b>{stage(ev.news?.label)}</b></span><span><i>Catalysts</i><b>{freshness(sections.catalysts)} · {cc.catalysts?.count??0}</b></span><span><i>Catalyst urgency</i><b>{stage(ev.catalysts?.urgency)}</b></span><span><i>Flow</i><b>{freshness(sections.flow)} · {cc.flow?.count??0}</b></span><span><i>Flow evidence</i><b>{stage(ev.flow?.label)}</b></span><span><i>Verification</i><b>{stage(c.verification_status)}</b></span></div>{(ev.news?.reason||ev.catalysts?.reason||ev.flow?.reason)&&<div className="funnel-context-block"><b>Evidence interpretation</b>{ev.news?.reason&&<span>News: {ev.news.reason}</span>}{ev.catalysts?.reason&&<span>Catalysts: {ev.catalysts.reason}</span>}{ev.flow?.reason&&<span>Flow: {ev.flow.reason}</span>}<span>These labels are descriptive context only and do not change the Opportunity formula score.</span></div>}{(cc.catalysts?.upcoming||[]).length>0&&<div className="funnel-context-block"><b>Upcoming catalysts</b>{cc.catalysts.upcoming.map((x:any,i:number)=><span key={i}>{x.date||"—"} · {x.title||x.kind||"Catalyst"} · {x.impact||"unrated"}</span>)}</div>}{(cc.news?.top||[]).length>0&&<div className="funnel-context-block"><b>Linked news</b>{cc.news.top.map((x:any,i:number)=><span key={i}>{x.title||"Untitled"}</span>)}</div>}{(cc.flow?.top||[]).length>0&&<div className="funnel-context-block"><b>Recent flow</b>{cc.flow.top.map((x:any,i:number)=><span key={i}>{x.label||x.description||x.symbol||"Flow observation"}</span>)}</div>}<div className="funnel-explain">{Object.values(c.explain||{}).map((v:any,i)=><p key={i}>{String(v)}</p>)}</div><button onClick={()=>onOpen(c.symbol)}>Open unified research</button></div></details>})}</div>
  </>}
