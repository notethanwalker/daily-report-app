...
def _impact(title: str) -> tuple[str, int]:
    t = title.lower()
...
def _category(title: str, source: str) -> str:
    t = title.lower()
    if "fomc" in t or "federal reserve" in t or "beige book" in t or "interest rate" in t:
        return "Central banks"
    if any(k in t for k in ["consumer price", "producer price", "inflation", "pce", "price index"]):
        return "Inflation"
    if any(k in t for k in ["employment", "job openings", "jolts", "unemployment", "earnings", "wages"]):
        return "Labor"
    if any(k in t for k in ["gdp", "personal income", "corporate profits", "productivity"]):
        return "Growth"
    if "trade" in t or "international" in t:
        return "Trade"
    if source == "Earnings cache":
        return "Earnings"
    return "Economic data"


def _event(event_date: str, title: str, source: str, source_url: str, *, event_time: str | None = None, symbol: str | None = None, detail: str | None = None) -> dict:
...
def bls_events(start: date, end: date) -> list[dict]:
    with httpx.Client(timeout=20, follow_redirects=True) as client:
        r = client.get(BLS_ICS, headers={"User-Agent": "DailyReportApp/1.9"})
        r.raise_for_status()
    blocks: list[dict] = []
    ...
    for item in blocks:
        ...
        event_time = None
        if len(digits) >= 12:
            event_time = f"{digits[8:10]}:{digits[10:12]}"
...
def bea_events(start: date, end: date) -> list[dict]:
    with httpx.Client(timeout=20, follow_redirects=True) as client:
        r = client.get(BEA_SOURCE, headers={"User-Agent": "DailyReportApp/1.9"})
        r.raise_for_status()
    p=_TableParser();p.feed(r.text);out=[]
    month_names="|".join(calendar.month_name[1:])
    rx=re.compile(rf"({month_names})\s+(\d{{1,2}})(?:\s+(\d{{1,2}}:\d{{2}}\s*[AP]M))?",re.I)
    for row in p.rows:
        joined=" | ".join(row)
        m=rx.search(joined)
        if not m:
            continue
        try:d=datetime.strptime(f"{m.group(1)} {m.group(2)} {start.year}","%B %d %Y").date()
        except ValueError:continue
        if not start <= d <= end:
            continue
...
# Official FOMC meeting dates currently published for 2026-2027. Keeping them explicit avoids brittle HTML parsing.
_FOMC = {
    2026: [(9,15,16,True),(10,27,28,False),(12,8,9,True)],
    2027: [(1,26,27,False),(3,16,17,True),(4,27,28,False),(6,8,9,True),(7,27,28,False),(9,14,15,True),(10,26,27,False),(12,7,8,True)],
}


def fed_events(start: date, end: date) -> list[dict]:
    out=[]
    for year, meetings in _FOMC.items():
        for month, day1, day2, sep in meetings:
            d=date(year,month,day2)
            if start <= d <= end:
                detail=f"FOMC two-day meeting {calendar.month_name[month]} {day1}-{day2}. Policy statement and press conference are normally on the second day."
                if sep:detail += " Meeting includes Summary of Economic Projections."
                out.append(_event(d.isoformat(), "FOMC policy decision", "Federal Reserve", FED_SOURCE, event_time="14:00", detail=detail))
                out.append(_event(d.isoformat(), "FOMC press conference", "Federal Reserve", FED_SOURCE, event_time="14:30", detail="Federal Reserve Chair press conference following the policy decision."))
                minutes=d+timedelta(days=21)
                if start <= minutes <= end:
                    out.append(_event(minutes.isoformat(), "FOMC meeting minutes", "Federal Reserve", FED_SOURCE, event_time="14:00", detail="Minutes are generally released three weeks after a scheduled FOMC meeting."))
    return out


def macroradar_events(start: date, end: date) -> list[dict]:
    try:
        payload=macro_calendar(start.isoformat(),end.isoformat())
    except Exception:
        return []
    out=[]
...
def catalog(start: date, end: date) -> dict:
    ...
    dedup={}
    for e in events:
        key2=(e.get("event_date"),str(e.get("title") or "").lower(),e.get("symbol"))
        old=dedup.get(key2)
        if old is None or int(e.get("impact_score") or 0)>int(old.get("impact_score") or 0):dedup[key2]=e
    result={"events":list(dedup.values()),"providers":providers,"errors":errors,"retrieved_at":datetime.now(timezone.utc).isoformat()}
