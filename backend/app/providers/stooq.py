...
def _parse_archive_row(row: dict) -> dict | None:
    ...
    if not dt or close is None:
        return None
    return {"date":dt[:10],"open":_number(row.get("<OPEN>") or row.get("OPEN")),"high":_number(row.get("<HIGH>") or row.get("HIGH")),"low":_number(row.get("<LOW>") or row.get("LOW")),"close":close,"volume":_number(row.get("<VOL>") or row.get("VOL")) or 0.0}
    ...
    def daily_history(self, symbol: str) -> dict:
        ...
        try:
            with httpx.Client(timeout=30.0, follow_redirects=True) as client:
                response = client.get(BASE_URL,params={"s":_stooq_symbol(s),"i":"d"},headers={"User-Agent":"daily-report-app/1.0"})
        ...
        try:
            for row in csv.DictReader(StringIO(text)):
                dt=str(row.get("Date") or "")[:10];close=_number(row.get("Close"))
                if not dt or close is None:continue
                rows.append({"date":dt,"open":_number(row.get("Open")),"high":_number(row.get("High")),"low":_number(row.get("Low")),"close":close,"volume":_number(row.get("Volume")) or 0.0})
        except Exception as exc:
            raise StooqError(f"Stooq returned malformed history for {s}") from exc
        if len(rows)<14:raise StooqError(f"Stooq returned insufficient history for {s}")
        rows.sort(key=lambda x:x["date"])
        return {"symbol":s,"rows":rows,"provider":self.name,"source_url":SOURCE_URL,"retrieved_at":datetime.now(timezone.utc).isoformat()}

    def download_us_bulk_archive(self) -> str:
        """Stream the Stooq U.S. daily archive to disk instead of holding it in RAM."""
        fd,path=tempfile.mkstemp(prefix="stooq-us-",suffix=".zip");os.close(fd)
        try:
            with httpx.Client(timeout=httpx.Timeout(30.0,read=300.0),follow_redirects=True) as client:
                with client.stream("GET",BULK_US_URL,headers={"User-Agent":"daily-report-app/1.0"}) as response:
                    ...
                    with open(path,"wb") as out:
                        for chunk in response.iter_bytes(chunk_size=1024*1024):out.write(chunk)
            try:
                with ZipFile(path) as zf:
                    names=zf.namelist()
                    if not names or not any(name.lower().endswith(".txt") for name in names):raise StooqError("Stooq bulk archive contains no data files")
    ...
    def iter_us_bulk_history(self, archive_path: str, tail: int = 260):
        ...
        with ZipFile(archive_path) as zf:
            for name in zf.namelist():
                ...
                yield {"symbol":symbol,"rows":list(rows),"provider":self.name,"source_url":BULK_US_URL,"retrieved_at":datetime.now(timezone.utc).isoformat()}
