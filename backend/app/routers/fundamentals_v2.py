...
def _load_sources(symbol:str,db:Session,market:dict)->tuple[dict,list[str]]:
    ...
    # Yahoo is the primary no-key enrichment path. It typically gives the richest trailing/forward data.
    ...
    # Use Alpha Vantage only when still incomplete and quota remains; this protects the 25/day free allowance.
    ...
    merged["retrieved_at"]=datetime.now(timezone.utc).isoformat();merged["valuation_refresh_version"]=4
    merged=stable._enrich_fundamental_ratios(merged,market)
    if "SEC EDGAR" in merged.get("valuation_sources",[]):merged["valuation_note"]="SEC EDGAR is used only to fill missing standardized revenue, EPS or share-count facts; Yahoo/Alpha fields are preferred when available."
...
def fundamentals(symbol:str,db:Session=Depends(get_db)):
    ...
    if cached:
        ...
        if age<stable.FUNDAMENTAL_CACHE_TTL_SECONDS and int((cached.payload or {}).get("valuation_refresh_version") or 0)>=4 and _complete(p):
            return {**p,"fundamentals_cache":"fresh","coverage":{"pe":p.get("pe_ratio") is not None,"ps":p.get("price_to_sales_ratio") is not None,"peg":p.get("peg_ratio") is not None}}
    ...
    db.commit()
    return {**fresh,"fundamentals_cache":"fresh","fundamentals_errors":errors,"coverage":{"pe":fresh.get("pe_ratio") is not None,"ps":fresh.get("price_to_sales_ratio") is not None,"peg":fresh.get("peg_ratio") is not None}}
