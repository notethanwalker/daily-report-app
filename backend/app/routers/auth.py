...
def _clear_rate(store: dict[str, deque[float]], key: str) -> None:
    store.pop(key, None)
...
def register(body: Credentials, request: Request, db: Session = Depends(get_db)):
    ...
    if not existing:
        account = AuthAccount(
            ...
            email_ciphertext=encrypt_text(email),
            email_lookup=lookup,
        ...
        db.commit()
        db.refresh(account)
        notification_sent = notify_owner_registration(db, account)
    elif existing.status == "pending":
        notification_sent = notify_owner_registration(db, existing)
    # Deliberately return the same public shape for existing addresses to reduce account enumeration.
    return {
        "status": "pending",
        "message": "Your account request has been submitted for administrator approval.",
...
def login(body: Credentials, request: Request, response: Response, db: Session = Depends(get_db)):
    try:
        ...
        lookup = "invalid"
    rate_key = _client_key(request, lookup[:16])
    _rate_limit(_login_attempts, rate_key, 10, 900)
    ...
    if account.status == "rejected" or not account.enabled:
        raise HTTPException(403, "Account access is not enabled")
...
def update_profile(body: ProfileUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    if body.email is not None:
        ...
        if new_email != current_email:
            ...
            new_lookup = email_lookup(new_email)
            duplicate = db.query(AuthAccount).filter(AuthAccount.email_lookup == new_lookup, AuthAccount.id != account.id).first()
            if duplicate:
                raise HTTPException(409, "That email address is already registered")
            account.email_ciphertext = encrypt_text(new_email)
            account.email_lookup = new_lookup
    db.commit()
    return {"account": safe_account(account)}
...
def pending_accounts(request: Request, db: Session = Depends(get_db)):
    _owner_or_403(request, db)
    rows = db.query(AuthAccount).filter(AuthAccount.status == "pending").order_by(AuthAccount.created_at.asc()).all()
    return {
        "accounts": [
            {
                "id": row.id,
                "email": decrypt_text(row.email_ciphertext),
                "created_at": row.created_at.isoformat() if row.created_at else None,
            }
            for row in rows
        ],
        "approval_email_notifications_configured": mail_configured(),
    }
...
def approve_account(user_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    notification_sent = notify_user_decision(db, account, True)
...
def reject_account(user_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.query(AuthSession).filter(AuthSession.user_id == account.id).delete(synchronize_session=False)
    db.commit()
    notification_sent = notify_user_decision(db, account, False)
