...
def _state(level: float, delta: float, trend: float) -> tuple[str, str]:
    if level >= 1.0 and delta > .35:return "leading_accelerating", "inflow_candidate"
    if level >= 1.0 and delta < -.35:return "leading_weakening", "rotation_out_risk"
    if level >= 0:return "leading_stable", "hold_leadership"
    if level < -1.0 and delta > .35:return "lagging_improving", "early_rotation_candidate"
    if level < -1.0 and delta < -.35:return "lagging_deteriorating", "avoidance_bias"
    if trend > 0 and delta > 0:return "recovering", "watch_for_rotation"
    return "lagging_stable", "neutral_to_weak"


def _history_record(rows: list[dict], day: str) -> dict:
    return {"date": day,"rows": [{"symbol":r["symbol"],"name":r["name"],"rotation_score":r["rotation_score"],"rotation_pressure":r["rotation_pressure"],"state":r["state"],"forward_bias":r["forward_bias"],"conviction":r["conviction"],"stale_input":r["stale_input"]} for r in rows]}


def _persist_history(db: Session, rows: list[dict], generated_at: str) -> bool:
    day=generated_at[:10];state=db.get(MarketPipelineState,ROTATION_HISTORY_KEY);payload=dict(state.payload or {}) if state else {};history=list(payload.get("daily") or []);record=_history_record(rows,day)
    previous_today=next((x for x in history if x.get("date")==day),None)
    if previous_today==record and payload.get("methodology_version")==ROTATION_MODEL_VERSION:return False
    history=[x for x in history if x.get("date")!=day];history.append(record);history=sorted(history,key=lambda x:x.get("date",""))[-MAX_ROTATION_HISTORY_DAYS:]
    payload.update({"daily":history,"latest_date":day,"methodology_version":ROTATION_MODEL_VERSION})
    if state:state.payload=payload
    else:db.add(MarketPipelineState(key=ROTATION_HISTORY_KEY,payload=payload))
...
def _observation_age_hours(row: MarketSnapshot, payload: dict, now: datetime) -> float:
    ages=[_age_hours(row.retrieved_at,now)];raw=str(payload.get("as_of") or row.as_of or "").strip()
    if raw:
        try:
            text=raw.replace("Z","+00:00")
            if len(text)==10:observed=datetime.fromisoformat(text).replace(tzinfo=timezone.utc)
            else:
                observed=datetime.fromisoformat(text);observed=observed.replace(tzinfo=timezone.utc) if observed.tzinfo is None else observed.astimezone(timezone.utc)
...
def build_rotation_model(db: Session, persist: bool = False) -> dict:
    ...
    for symbol,name in SECTORS.items():
        ...
        if diffs:
            sign=1 if delta_3>=0 else -1;direction_consistency=sum(1 for x in diffs[-3:] if (x>=0 and sign>0) or (x<=0 and sign<0))/min(3,len(diffs))
        trend_agrees=1.0 if (level>=0 and trend>=0) or (level<0 and trend<0) else .5;raw_conviction=min(100.0,25+min(abs(level)*8,30)+min(abs(delta_3)*12,25)+direction_consistency*10+trend_agrees*5);history_quality=min(1.0,len(scores)/8.0);conviction=raw_conviction*(0.45+0.55*history_quality);transition_ready=len(scores)>=MIN_TRANSITION_OBSERVATIONS;data_age_hours=_observation_age_hours(latest_row,p,now);stale_input=data_age_hours>STALE_INPUT_HOURS
        if not transition_ready:
            conviction=min(conviction,45.0)
            if forward_bias not in {"hold_leadership","neutral_to_weak"}:forward_bias="insufficient_history"
        if stale_input:conviction=min(conviction,35.0);forward_bias="stale_input"
        pressure=level+delta_3*1.25
        rows.append({"symbol":symbol,"name":name,"rotation_score":round(level,3),"delta_1_observation":round(delta_1,3),"delta_3_observations":round(delta_3,3),"rotation_pressure":round(pressure,3),"state":state,"forward_bias":forward_bias,"conviction":round(conviction,1),"transition_ready":transition_ready,"history_quality":round(history_quality,2),"data_age_hours":round(data_age_hours,1),"stale_input":stale_input,"trend_context":round(trend,2),"relative_volume":_f(p.get("relative_volume")),"observations":len(scores),"as_of":p.get("as_of") or latest_row.as_of})
...
async def rotation_snapshot_loop():
    while True:
        db=SessionLocal()
        try:build_rotation_model(db,persist=True)
