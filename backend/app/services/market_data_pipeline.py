...
def _upsert_bar_payloads(db: Session, payloads: list[dict]):
    ...
    stmt = stmt.on_conflict_do_update(
        ...
        set_={
            ...
            "provider": stmt.excluded.provider, "source_url": stmt.excluded.source_url,
...
def persist_normalized_history(db: Session, data: dict) -> int:
    symbol=data["symbol"].upper(); provider=str(data.get("provider") or "Unknown"); source_url=str(data.get("source_url") or "")
    payloads=[]
...
def _snapshot_from_normalized(db: Session, symbol: str) -> dict | None:
    rows=db.query(NormalizedDailyBar).filter(NormalizedDailyBar.symbol==symbol.upper(),NormalizedDailyBar.high.is_not(None),NormalizedDailyBar.low.is_not(None)).order_by(NormalizedDailyBar.bar_date.desc()).limit(260).all()
...
def refresh_symbol(db: Session, symbol: str, tracked: bool = False) -> dict:
    ...
    touched=persist_normalized_history(db,data);snapshot=_snapshot_from_normalized(db,symbol)
    if snapshot:
        db.add(MarketSnapshot(symbol=symbol.upper(),as_of=str(snapshot.get("as_of") or ""),provider=str(snapshot.get("provider") or data.get("provider") or "Normalized cache"),payload=snapshot));db.commit()
...
def _nasdaq_universe_symbols(db: Session) -> set[str]:
    rows=db.query(SymbolRegistry).filter(or_(SymbolRegistry.asset_type.in_(["Stock","ETF","Equity"]),SymbolRegistry.asset_type.is_(None))).all()
    return {r.symbol.upper() for r in rows if (r.provider_ids or {}).get("universe_source")=="Nasdaq Trader"}
...
def bulk_refresh_us_market(db: Session, force_full: bool = False) -> dict:
    """Download one Stooq US archive and update the shared cache without per-symbol provider calls.

    Initial bootstrap persists the latest 260 bars per symbol. Later runs persist only
    the latest 5 bars for corrections/new sessions while still calculating technical
    snapshots directly from each archive member's 260-bar tail.
    """
    ...
    try:
        ...
        if universe==0:
            sync_us_symbol_universe(db);universe,covered=_coverage(db)
        full=force_full or covered < max(100,int(universe*0.60))
        persist_tail=260 if full else 5
        symbols=_nasdaq_universe_symbols(db)
        provider=StooqProvider();archive=provider.download_us_bulk_archive()
        ...
        for data in provider.iter_us_bulk_history(archive,tail=260):
            ...
            for item in (data.get("rows") or [])[-persist_tail:]:
                bar_buffer.append({"symbol":symbol,"bar_date":item["date"],"open":_number(item.get("open")),"high":_number(item.get("high")),"low":_number(item.get("low")),"close":_number(item.get("close")),"volume":_number(item.get("volume")) or 0.0,"provider":"Stooq","source_url":data.get("source_url") or ""})
            snapshot_buffer.append(MarketSnapshot(symbol=symbol,as_of=str(snapshot.get("as_of") or ""),provider="Stooq",payload=snapshot))
            if len(bar_buffer)>=10000:
                _upsert_bar_payloads(db,bar_buffer);bar_buffer=[]
            if len(snapshot_buffer)>=500:
                db.add_all(snapshot_buffer);snapshot_buffer=[];db.commit()
        _upsert_bar_payloads(db,bar_buffer);db.add_all(snapshot_buffer);db.commit()
        return {"status":"complete","mode":"full_bootstrap" if full else "daily_incremental","archive_symbols_seen":seen,"universe_symbols":universe,"eligible_symbols":eligible,"persisted_days_per_symbol":persist_tail,"provider":"Stooq bulk US daily archive","completed_at":datetime.now(timezone.utc).isoformat()}
    finally:
        if archive:
...
def bootstrap_market_batch(db: Session, limit: int = 8) -> dict:
    """Fallback repair path only; broad bootstrap now uses the Stooq archive."""
