...
CRITERIA = {
    "williams": {"key":"williams","label":"Williams %R","description":"Rewards deeper oversold conditions. Default-model primary signal.","raw_field":"williams_r_14","higher_score_is_better":True,"min_sessions":14,"coverage_class":"broad_cache_safe","hypothesis":"Deep short-term oversold states can create asymmetric mean-reversion entries when other setup conditions are supportive."},
    "ma100_proximity": {"key":"ma100_proximity","label":"100MA proximity","description":"Rewards price approaching the 100-day moving average from above.","raw_field":"price_vs_ma100_percent","higher_score_is_better":True,"min_sessions":100,"coverage_class":"broad_cache_safe","hypothesis":"Pullbacks toward an established medium-term trend reference can improve entry asymmetry without requiring a trend break."},
    "ma50_proximity": {"key":"ma50_proximity","label":"50MA proximity","description":"Rewards price approaching the 50-day moving average from above using the same proximity transform as the 100MA factor.","raw_field":"price_vs_ma50_percent","higher_score_is_better":True,"min_sessions":50,"coverage_class":"broad_cache_safe","hypothesis":"A pullback toward shorter intermediate-trend support may identify earlier entries than the 100MA while retaining trend context."},
...
def _relative_volume_score(value:float)->float:return max(0.0,min(100.0,((value-0.5)/1.5)*100.0))

def validate_formula(criteria:dict|None)->dict[str,float]:
    source=DEFAULT_FORMULA if criteria is None else criteria
    if not isinstance(source,dict) or not source:raise ValueError("Formula must contain at least one criterion")
    clean={}
    for key,weight in source.items():
        if key not in CRITERIA:raise ValueError(f"Unknown Opportunity criterion: {key}")
        try:number=float(weight)
        except (TypeError,ValueError) as exc:raise ValueError(f"Invalid weight for {key}") from exc
        if number<=0 or number>1000:raise ValueError(f"Weight for {key} must be greater than 0 and no more than 1000")
...
def validate_filters(filters:list[dict]|None)->list[dict]:
    ...
    for item in filters or []:
        if not isinstance(item,dict):raise ValueError("Opportunity filters must be objects")
        field=str(item.get("field") or "").strip();operator=str(item.get("operator") or "").strip()
        if field not in FILTER_FIELDS:raise ValueError(f"Unknown Opportunity filter field: {field}")
        if operator not in FILTER_FIELDS[field]["operators"]:raise ValueError(f"Unsupported operator for {field}: {operator}")
        try:value=float(item.get("value"))
        except (TypeError,ValueError) as exc:raise ValueError(f"Invalid filter value for {field}") from exc
        clean.append({"field":field,"operator":operator,"value":round(value,6)})
    if len(clean)>12:raise ValueError("Opportunity formulas support at most 12 hard filters")
    return clean

def validate_sort(sort_by:str,sort_dir:str)->tuple[str,str]:
    by=str(sort_by or "score").strip().lower();direction=str(sort_dir or "desc").strip().lower()
    if by not in SORT_FIELDS:raise ValueError(f"Unsupported Opportunity sort field: {by}")
    if direction not in SORT_DIRECTIONS:raise ValueError(f"Unsupported Opportunity sort direction: {direction}")
    return by,direction
...
def formula_score(components:dict[str,float|None],criteria:dict|None)->float|None:
    ...
    if any(components.get(key) is None for key in weights):return None
    denominator=sum(weights.values())
    if denominator<=0:return None
    return round(sum(float(components[key])*weight for key,weight in weights.items())/denominator,2)
...
def passes_filters(payload:dict,filters:list[dict]|None)->bool:
    for item in validate_filters(filters):
        ...
        if item["operator"]=="<=" and not observed<=item["value"]:return False
        if item["operator"]==">=" and not observed>=item["value"]:return False
    return True
...
def sort_index_rows(rows:list[dict],sort_by:str="score",sort_dir:str="desc")->tuple[list[dict],str,str]:
    by,direction=validate_sort(sort_by,sort_dir);present=[item for item in rows if _sort_value(item,by) is not None];missing=[item for item in rows if _sort_value(item,by) is None];reverse=direction=="desc"
    if by=="symbol":present.sort(key=lambda item:str(_sort_value(item,by)),reverse=reverse)
    else:present.sort(key=lambda item:(float(_sort_value(item,by)),item.get("symbol") or ""),reverse=reverse)
    ordered=present+missing
    for position,item in enumerate(ordered,1):item["display_position"]=position
...
def build_opportunity_index(db:Session,*,criteria:dict|None=None,filters:list[dict]|None=None,include_etfs:bool=False,limit:int=300,sort_by:str="score",sort_dir:str="desc",target_symbols:list[str]|set[str]|None=None)->dict:
    ...
    for row in latest_rows:
        ...
        scannable+=1;newest=row.retrieved_at if newest is None or row.retrieved_at>newest else newest;price=_f(payload.get("price"));williams=_f(payload.get("williams_r_14"));ma100_distance=_f(payload.get("price_vs_ma100_percent"));avg_dollar_volume=_f(payload.get("average_dollar_volume_20d"))
        if price is None or williams is None or ma100_distance is None:continue
