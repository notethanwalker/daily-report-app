...
def encrypt_text(value: str) -> str:
    nonce = os.urandom(12)
    ciphertext = AESGCM(_required_key("AUTH_ENCRYPTION_KEY")).encrypt(nonce, value.encode("utf-8"), None)
...
def decrypt_text(value: str | None) -> str | None:
    ...
    plaintext = AESGCM(_required_key("AUTH_ENCRYPTION_KEY")).decrypt(raw[:12], raw[12:], None)
    return plaintext.decode("utf-8")


def email_lookup(email: str) -> str:
    normalized = normalize_email(email)
    return hmac.new(_required_key("AUTH_LOOKUP_KEY"), normalized.encode("utf-8"), hashlib.sha256).hexdigest()
...
def hash_password(password: str) -> str:
    ...
    return f"scrypt${PASSWORD_N}${PASSWORD_R}${PASSWORD_P}${_b64e(salt)}${_b64e(derived)}"
...
def verify_password(password: str, encoded: str) -> bool:
    try:
        ...
        actual = hashlib.scrypt(
            password.encode("utf-8"), salt=_b64d(salt_text), n=int(n), r=int(r), p=int(p), dklen=len(expected)
        )
...
def _session_digest(token: str) -> str:
    return hmac.new(_required_key("AUTH_SESSION_PEPPER"), token.encode("utf-8"), hashlib.sha256).hexdigest()
...
def account_from_session(db: Session, token: str | None) -> AuthAccount | None:
    ...
    if not account or not account.enabled or account.status != "approved":
        return None
...
def safe_account(account: AuthAccount) -> dict:
    return {
        "id": account.id,
        "email": decrypt_text(account.email_ciphertext),
        "name": decrypt_text(account.name_ciphertext),
        "name_required": not bool(account.name_ciphertext),
        "role": account.role,
...
def ensure_user_defaults(db: Session, account: AuthAccount) -> None:
    ...
    if not profile:
        db.add(UserProfile(email=account.id, role=account.role, enabled=account.enabled))
    else:
        profile.role = account.role
        profile.enabled = account.enabled

    access = db.get(UserAccessConfig, account.id)
    if not access:
        db.add(UserAccessConfig(user_email=account.id, enabled=account.enabled, role=account.role, permissions={}, allowed_tabs=[]))
    else:
        access.enabled = account.enabled
        access.role = account.role
        access.token_hash = None

    if not db.get(UserPreferences, account.id):
        db.add(UserPreferences(user_email=account.id, visible_tabs=[], information_modules={}, settings={}))

    initialized = db.query(UserWatchlistItem).filter(
        UserWatchlistItem.user_email == account.id, UserWatchlistItem.symbol == SENTINEL
    ).first()
    if not initialized:
        ...
        for item in db.query(WatchlistItem).order_by(WatchlistItem.created_at).all():
            exists = db.query(UserWatchlistItem).filter(
                UserWatchlistItem.user_email == account.id, UserWatchlistItem.symbol == item.symbol
            ).first()
            if not exists:
                db.add(UserWatchlistItem(user_email=account.id, symbol=item.symbol))
    db.commit()


def bootstrap_admin(db: Session) -> AuthAccount | None:
    user_id = os.getenv("AUTH_BOOTSTRAP_ADMIN_ID", "").strip()
    email_ciphertext = os.getenv("AUTH_BOOTSTRAP_ADMIN_EMAIL_CIPHERTEXT", "").strip()
    lookup = os.getenv("AUTH_BOOTSTRAP_ADMIN_EMAIL_LOOKUP", "").strip()
    password_hash = os.getenv("AUTH_BOOTSTRAP_ADMIN_PASSWORD_HASH", "").strip()
    if not all([user_id, email_ciphertext, lookup, password_hash]):
        return None
    existing = db.get(AuthAccount, user_id) or db.query(AuthAccount).filter(AuthAccount.email_lookup == lookup).first()
    if existing:
        ensure_user_defaults(db, existing)
        return existing
    now = datetime.now(timezone.utc)
    account = AuthAccount(
        id=user_id,
