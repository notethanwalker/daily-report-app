...
from .candidate_funnel_v4 import _f, _latest_feature_map, _latest_market_map, _setup_type, _shortlist_bars
...
FORMULA_FUNNEL_MODEL_VERSION = "candidate-funnel-formula-v4.3"
SOURCE_MULTIPLIER = 4
MIN_SOURCE_ROWS = 200
...
def _apply_flow_confirmation_guardrail(item: dict) -> None:
    """Flow can constrain decision readiness, but never rewrite formula score/rank."""
    flow = ((item.get("candidate_context") or {}).get("persistent_flow") or {})
    verdict = str(flow.get("verdict") or "insufficient")
    confidence = str(flow.get("confidence") or "none")
    item["flow_confirmation"] = {
        "verdict": verdict,
        "direction": flow.get("direction") or "unknown",
        "confidence": confidence,
        "score_effect": 0,
    }
    item["decision_warning"] = None
    if verdict == "contradiction" and confidence == "medium":
        item["decision_warning"] = "Medium-confidence persistent bearish flow contradicts the long setup. Formula score and rank are unchanged."
        if item.get("attention_stage") == "high_conviction":
            item["attention_stage"] = "actionable"
            item["flow_guardrail_applied"] = True
    elif verdict == "mixed":
        item["decision_warning"] = "Persistent flow is directionally mixed; do not treat options activity as confirmation."
...
def build_formula_candidate_funnel(db: Session, rotation: dict, *, criteria: dict | None = None, filters: list[dict] | None = None, limit: int = 50, enqueue_enrichment: bool = False) -> dict:
    source_limit = min(1000, max(MIN_SOURCE_ROWS, limit * SOURCE_MULTIPLIER))
    index = build_opportunity_index(db, criteria=criteria, filters=filters, include_etfs=False, limit=source_limit, sort_by="score", sort_dir="desc")
    source = list(index.get("rows") or [])
    source_symbols = [row["symbol"] for row in source]
    registry = {x.symbol.upper(): x for x in db.query(SymbolRegistry).filter(SymbolRegistry.symbol.in_(source_symbols)).all()} if source_symbols else {}
    ...
    for row in source:
        ...
        if feature:
            base_buy = _f(feature.get("buy_score"), 50.0); base_source = "persisted_opportunity_model"; feature_context = "available"
        else:
            base_buy = 50.0; base_source = "neutral_without_feature_snapshot"; feature_context = "missing"
        contextual = _context_score(formula_score, macro_fit, base_buy, bool(feature))
        raw_criteria = dict(row.get("raw_criteria") or {})
        ranked.append({
            "symbol": symbol, "name": row.get("name") or (reg.name if reg else None), "sector": sector, "industry": industry,
            "formula_rank": row.get("formula_rank"), "formula_score": round(formula_score, 2), "contextual_priority_score": round(contextual, 2),
            "criterion_scores": row.get("criterion_scores") or {}, "raw_criteria": raw_criteria,
            "rotation_proxy": macro.get("dominant_proxy"), "rotation_proxy_basis": macro.get("basis"), "rotation_exposures": macro.get("exposures"),
            "rotation_components": macro.get("components"), "setup_type": _setup_type(macro), "base_buy_score": round(base_buy, 1), "base_buy_source": base_source,
            "rotation_pressure": round(pressure, 3), "rotation_state": macro.get("state"), "rotation_conviction": round(conviction, 1),
            "macro_confidence_factor": round(confidence, 3), "williams_r_14": raw_criteria.get("williams"), "price_vs_ma100_percent": raw_criteria.get("ma100_proximity"),
            "average_dollar_volume_20d": row.get("average_dollar_volume_20d"), "price": row.get("price"), "as_of": row.get("as_of"),
            "provider": row.get("provider"), "source_url": row.get("source_url"), "verification_status": row.get("verification_status"),
            "feature_context_status": feature_context, "_market": market, "_feature": feature,
            "explain": {
    ...
    for priority_rank, item in enumerate(short, start=1):
        ...
        try:
            ...
            item["convergence"] = {k: conv.get(k) for k in (
                "state", "raw_state", "state_code", "convergence_score", "williams_r", "ma100_distance_pct", "ma200_distance_pct", "quality_score", "quality_detail",
                "confirmation_count", "confirmation_available", "confirmation_possible", "confirmation_evidence_ready", "price_context", "data_alignment",
                "missing_required_inputs", "as_of", "model_version", "model_config_hash", "alert_ready",
            )}
            state = str(conv.get("state") or "extended")
        except Exception as exc:
            item["convergence"] = {"state": "unavailable", "error": str(exc)[:160], "alert_ready": False}; state = "unavailable"
    ...
    for item in short:
        attention_counts[item["attention_stage"]] = attention_counts.get(item["attention_stage"], 0) + 1
    ...
    return {
        "model_version": FORMULA_FUNNEL_MODEL_VERSION, "formula": formula_metadata(criteria, filters), "source_index_counts": counts,
        "source_rows_considered": len(source), "candidates": short, "attention_counts": attention_counts, "convergence_counts": convergence_counts,
        "candidate_context": context_summary, "fundamental_enrichment_targets": fundamental_targets, "fundamental_jobs_added": queued_fundamentals,
        "deep_enrichment_symbols": fundamental_targets, "deep_enrichment_jobs_added": len(queued_fundamentals), "last_cache_update": index.get("last_cache_update"),
        "stages": [
            ...
            {"name": "Readiness", "input": len(short), "output": attention_counts["actionable"] + attention_counts["high_conviction"], "rule": "Classify Watch → Developing → Actionable → High Conviction; medium-confidence bearish persistent flow can block High Conviction without changing formula score/rank."},
            {"name": "Cached intelligence", "input": len(short), "output": context_summary.get("available", {}).get("news", 0), "rule": "Attach cached fundamentals, linked news, catalysts, SEC filings and live/stored flow without provider calls; freshness is reported separately."},
        ...
        "methodology": "Formula-first candidate funnel. Formula score/rank define asset opportunity. Context can refine decision readiness but cannot rewrite formula rank. Liquidity is a gate, never a bonus. Persistent options flow is confirmation/contradiction context only; medium-confidence bearish flow may block a High Conviction presentation state while score_effect remains zero. Automatic ranking and shortlist refresh perform zero provider calls.",
