...
def build_candidate_funnel(db:Session,rotation:dict,limit:int=50,enqueue_enrichment:bool=False)->dict:
    ...
    ranked.sort(key=lambda x:x["raw_rank_score"],reverse=True);short=ranked[:limit]
    convergence_counts={"extended":0,"watching":0,"approaching":0,"triggered":0,"invalidated":0}
    for item in short:
        try:
            conv=evaluate_convergence(db,item["symbol"])
            item["convergence"]={k:conv.get(k) for k in ("state","state_code","convergence_score","williams_r","ma100_distance_pct","ma200_distance_pct","quality_score","confirmation_count","confirmation_total","price_context","missing_required_inputs","as_of","model_version","model_config_hash","alert_ready")}
            state=str(conv.get("state") or "extended");convergence_counts[state]=convergence_counts.get(state,0)+1
        except Exception as exc:
            item["convergence"]={"state":"unavailable","error":str(exc)[:160],"alert_ready":False}
    deep=[x["symbol"] for x in short if x["needs_deep_enrichment"]][:DEEP_ENRICHMENT_LIMIT];queued=enqueue_deep_enrichment(db,deep) if enqueue_enrichment and deep else 0
    convergence_ready=sum(convergence_counts.get(x,0) for x in ("approaching","triggered"))
    return {"model_version":CANDIDATE_MODEL_VERSION,"stages":[{"name":"Universe","input":scan.get("counts",{}).get("cached_symbols_scanned",0),"output":scan.get("counts",{}).get("technically_eligible",0),"rule":"Cached equities only; minimum price and average-dollar-volume filters."},{"name":"Technical setup","input":scan.get("counts",{}).get("technically_eligible",0),"output":len(source),"rule":"Williams %R + approach to 100MA, preserving strong/weak/near buckets."},{"name":"Macro fit","input":len(source),"output":len(source),"rule":"Blend weighted sector/industry/theme rotation exposures; contribution is confidence-weighted."},{"name":"Rank","input":len(source),"output":len(short),"rule":"55% scanner technical, 25% persisted buy score, 15% confidence-weighted macro, plus bounded liquidity/setup bonuses."},{"name":"Convergence","input":len(short),"output":convergence_ready,"rule":"Stage finalists as Extended → Watching → Approaching → Triggered → Invalidated using Williams reset, 100MA approach from above, quality, and confirmation filters."},{"name":"Deep enrichment shortlist","input":len(short),"output":len(deep),"rule":f"At most {DEEP_ENRICHMENT_LIMIT} scanner-only finalists are eligible for explicit fundamentals enrichment."}],"candidates":short,"convergence_counts":convergence_counts,"deep_enrichment_symbols":deep,"deep_enrichment_jobs_added":queued,"source_scan_counts":scan.get("counts",{}),"methodology":"Candidate-first/cache-first. Weighted theme exposure replaces one-proxy classification where configured. Scanner-only names receive a neutral nontechnical prior. Opportunity Convergence is a staged alert/readiness model and does not alter the base funnel rank.","generated_at":datetime.now(timezone.utc).isoformat()}
