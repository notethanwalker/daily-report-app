...
def _run_import_cycle(recovered_checked: bool) -> tuple[bool, int]:
    """Own the SQLAlchemy session entirely inside the worker thread.

    asyncio cancellation cannot stop a running to_thread call. Keeping the session
    in this synchronous scope prevents the event-loop task from closing it while
    SQLAlchemy is still acquiring/using a connection during a deployment shutdown.
    """
    db = SessionLocal()
    delay = 60
    try:
        if not recovered_checked:
            recovered = _recover_legacy_upload(db)
            if not recovered:
                recovered = _recover_remote_share(db)
            if not recovered:
                _mark_awaiting_reupload(db)
        ...
        db.close()
    return recovered_checked, delay
...
async def stooq_import_loop() -> None:
    ...
    while True:
        recovered_checked, delay = await asyncio.to_thread(_run_import_cycle, recovered_checked)
