...
def _technical_reasons(symbol: str, name: str, item: dict, rank_kind: str) -> list[dict]:
    ...
    trend_parts = []
    if vs50 is not None:
        trend_parts.append(f"50DMA {vs50:+.1f}%")
    if vs100 is not None:
        trend_parts.append(f"100DMA {vs100:+.1f}%")
    if vs200 is not None:
        trend_parts.append(f"200DMA {vs200:+.1f}%")
    ...
    if aligned_up or aligned_down:
        ...
        out.append({
            ...
            "title": f"Moving-average trend alignment",
            "matched_symbol": symbol,
            "matched_group": name,
            "summary": f"{symbol} is {direction} all tracked moving averages.",
    ...
    if rel_vol is not None and rel_vol >= 1.35:
        out.append({
            "reason_type": "technical",
            "title": "Elevated participation",
            "matched_symbol": symbol,
            "matched_group": name,
            "summary": f"Relative volume is {rel_vol:.2f}x normal.",
    ...
    if week is not None and month is not None and abs(week) >= 3 and abs(month) >= 5 and week * month > 0:
        out.append({
            ...
            "matched_symbol": symbol,
            "matched_group": name,
            "summary": f"7D and 30D momentum are aligned ({week:+.1f}% / {month:+.1f}%).",
    ...
    if price is not None and hi52 not in (None, 0):
        ...
        if from_high >= -2.5:
            out.append({
                "reason_type": "technical",
                "title": "Near 52-week resistance/high",
                "matched_symbol": symbol,
                "matched_group": name,
                "summary": f"Price is {abs(from_high):.1f}% below the 52-week high.",
    ...
    if price is not None and lo52 not in (None, 0):
        ...
        if from_low <= 5:
            out.append({
                "reason_type": "technical",
                "title": "Near 52-week support/low",
                "matched_symbol": symbol,
                "matched_group": name,
                "summary": f"Price is {from_low:.1f}% above the 52-week low.",
    ...
    if vs_ath is not None and vs_ath > -3:
        out.append({
            "reason_type": "technical",
            "title": "Close to provider-history all-time high",
            "matched_symbol": symbol,
            "matched_group": name,
            "summary": f"Price is {vs_ath:+.1f}% from the provider-history ATH.",
...
def _news_reasons(news: list[dict], focus_rows: list[dict]) -> list[dict]:
    ...
    for score, article, row in candidates:
        ...
        symbol = row["symbol"]
        url = article.get("url")
        if per_symbol[symbol] >= 3 or (url and (symbol, url) in seen_urls):
            continue
        per_symbol[symbol] += 1
        if url:
            seen_urls.add((symbol, url))
        confidence = "high" if score >= 75 else "medium" if score >= 45 else "low"
        title = article.get("title") or "Potential catalyst"
        ...
        out.append({
            ...
            "summary": article.get("why_it_matters") or f"Potential catalyst for {row['name']}.",
            "inference": article.get("why_it_matters") or f"Potential catalyst for {row['name']} based on ticker/theme overlap with the current move.",
...
def build_rotation_snapshot(market_by_symbol: dict[str, dict], currencies: dict | None = None, news: list[dict] | None = None, news_meta: dict | None = None) -> dict:
    ...
    leaders = rows[:8]
    laggards = list(reversed(rows[-8:])) if rows else []
    spread = None
    ...
    focus_rows = leaders + [row for row in laggards if row["symbol"] not in {x["symbol"] for x in leaders}]
    ...
    return {
        "methodology": "Rotation score = 25% of 1-day performance + 45% of 7-day performance + 30% of 30-day performance, then multiplied by relative volume up to 2x when volume is above normal. Positive scores indicate relative leadership; negative scores indicate relative weakness. It is a momentum ranking, not direct fund-flow measurement.",
        "sectors": rows,
        "leaders": leaders,
        "laggards": laggards,
        "spread": spread,
        ...
        "reasoning_methodology": "News catalysts are matched by ticker/theme token overlap, sector classification, relevance score, feed ordering, and source metadata. Technical explanations use moving-average alignment, relative volume, multi-timeframe momentum, and proximity to 52-week/provider-history highs and lows. These are explanatory candidates, not proof of causality.",
