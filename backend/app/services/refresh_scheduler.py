...
def _market_refresh_allowed(now):
    # Avoid wasting credits overnight/weekends. UTC window covers US regular session across DST plus a buffer.
...
def process_queue(db,limit=4):
    ...
    for idx,row in enumerate(rows):
        ...
        try:
            ...
            row.status="complete";row.error=None;db.commit();done.append(row.symbol)
