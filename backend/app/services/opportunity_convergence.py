...
MODEL_VERSION = "opportunity-convergence-v1.1"
...
MODEL_CONFIG = {
    ...
    "minimum_available_confirmations": 3,
}
...
def evaluate_convergence(db:Session,symbol:str)->dict:
    s=symbol.strip().upper();market,retrieved_at=_latest_market(db,s);feature,feature_as_of=_latest_feature(db,s);bars=_bars(db,s);ctx=_price_context(bars)
    ...
    confirmations={
        "relative_volume":_confirm(rv,lambda x:x>=1.0),
        "above_or_near_200ma":_confirm(ma200,lambda x:x>=-5.0),
        "rsi_reset":_confirm(ctx["rsi14"],lambda x:25.0<=x<=55.0),
        "meaningful_drawdown":_confirm(ctx["drawdown_60d_pct"],lambda x:x<=-5.0),
        "stabilizing":ctx["stabilizing"],
    }
    available_confirmations=sum(1 for val in confirmations.values() if val is not None);confirmation_count=sum(1 for val in confirmations.values() if val is True)
    ...
    evidence_ready=available_confirmations>=MODEL_CONFIG["minimum_available_confirmations"]
    triggered=(not missing and not invalid and evidence_ready and wr<=MODEL_CONFIG["williams_trigger"] and MODEL_CONFIG["ma100_trigger_range_pct"][0]<=ma100<=MODEL_CONFIG["ma100_trigger_range_pct"][1] and quality>=MODEL_CONFIG["quality_trigger_min"] and confirmation_count>=MODEL_CONFIG["minimum_confirmations"] and ctx["stabilizing"] is not False)
    approaching=(not missing and not invalid and wr<=MODEL_CONFIG["williams_approach"] and MODEL_CONFIG["ma100_approach_range_pct"][0]<=ma100<=MODEL_CONFIG["ma100_approach_range_pct"][1] and quality>=MODEL_CONFIG["quality_approach_min"])
    ...
    elif approaching:state="approaching"
    elif watching:state="watching"
    else:state="extended"
    wr_score=0.0 if wr is None else max(0.0,min(100.0,(-wr-40.0)/40.0*100.0));ma_score=0.0 if ma100 is None or ma100<-5 else max(0.0,min(100.0,100.0-abs(ma100-2.0)*8.0));q_score=max(0.0,min(100.0,quality or 0.0));conf_score=confirmation_count/max(available_confirmations,1)*100.0;score=round(wr_score*.35+ma_score*.30+q_score*.20+conf_score*.15,2)
    as_of=str(market.get("as_of") or feature_as_of or (bars[-1].bar_date if bars else ""))[:10] or None;event_key=f"{MODEL_VERSION}:{s}:{state}:{as_of}" if state=="triggered" and as_of else None
    return {"symbol":s,"state":state,"state_code":STATE_ORDER[state],"convergence_score":score,"williams_r":wr,"ma100_distance_pct":ma100,"ma200_distance_pct":ma200,"quality_score":quality,"relative_volume":rv,"confirmations":confirmations,"confirmation_count":confirmation_count,"confirmation_available":available_confirmations,"confirmation_possible":len(confirmations),"confirmation_evidence_ready":evidence_ready,"price_context":ctx,"missing_required_inputs":missing,"as_of":as_of,"retrieved_at":retrieved_at.isoformat() if retrieved_at else None,"model_version":MODEL_VERSION,"model_config_hash":MODEL_CONFIG_HASH,"model_config":MODEL_CONFIG,"event_key":event_key,"alert_ready":state=="triggered","interpretation":"Fundamentally/theme-attractive stock nearing a technically favorable entry. Williams reset is primary, 100MA approach from above is secondary, quality is a gate, and confirmations distinguish stabilization from an accelerating decline. Missing confirmation data is not treated as a failed confirmation."}
