...
def _quality(feature:dict):
    comps=feature.get("components") if isinstance(feature.get("components"),dict) else {};valuation=_f(comps.get("valuation"));sector=_f(comps.get("sector"));vals=[]
    if valuation is not None:vals.append((valuation,.55))
    if sector is not None:vals.append((sector,.45))
    if not vals:return None
    weight=sum(w for _,w in vals);return round(sum(v*w for v,w in vals)/weight,2)
def _quality_detail(feature:dict):
    comps=feature.get("components") if isinstance(feature.get("components"),dict) else {}
    return {"valuation":_f(comps.get("valuation")),"sector_theme":_f(comps.get("sector")),"excluded_from_quality_gate":["technical","momentum","flow","overall_buy_score"]}
...
def _alignment(market:dict,feature_as_of,bars,retrieved_at):
    market_day=_iso_date(market.get("as_of"));feature_day=_iso_date(feature_as_of);bar_day=_iso_date(bars[-1].bar_date) if bars else None;days=[x for x in (market_day,feature_day,bar_day) if x is not None];spread=(max(days)-min(days)).days if len(days)>=2 else None;age=None
    if retrieved_at:
    ...
    ready=len(days)>=2 and (spread is None or spread<=MODEL_CONFIG["maximum_input_date_spread_days"]) and (age is None or age<=MODEL_CONFIG["maximum_market_retrieval_age_hours"]);reasons=[]
...
def evaluate_convergence_inputs(symbol:str,market:dict,feature:dict,bars:list,retrieved_at=None,feature_as_of=None)->dict:
    s=symbol.strip().upper();ctx=_price_context(bars);alignment=_alignment(market,feature_as_of,bars,retrieved_at);wr=_f(market.get("williams_r_14"),_f(feature.get("williams_r")));ma100=_f(market.get("price_vs_ma100_percent"),_f(feature.get("ma100_distance")));ma200=_f(market.get("price_vs_ma200_percent"),_f(feature.get("ma200_distance")));rv=_f(market.get("relative_volume"),_f(feature.get("relative_volume")));quality=_quality(feature);quality_detail=_quality_detail(feature)
    confirmations={"relative_volume":_confirm(rv,lambda x:x>=1.0),"above_or_near_200ma":_confirm(ma200,lambda x:x>=-5.0),"rsi_reset":_confirm(ctx["rsi14"],lambda x:25.0<=x<=55.0),"meaningful_drawdown":_confirm(ctx["drawdown_60d_pct"],lambda x:x<=-5.0),"stabilizing":ctx["stabilizing"]};available_confirmations=sum(1 for val in confirmations.values() if val is not None);confirmation_count=sum(1 for val in confirmations.values() if val is True);missing=[name for name,value in (("williams_r",wr),("ma100_distance",ma100),("quality_score",quality)) if value is None];invalid=(ma100 is not None and ma100<MODEL_CONFIG["invalid_ma100_below_pct"]) or (quality is not None and quality<MODEL_CONFIG["invalid_quality_below"]);evidence_ready=available_confirmations>=MODEL_CONFIG["minimum_available_confirmations"]
    ...
    state="approaching" if raw_state=="triggered" and not alignment["ready_for_alert"] else raw_state;wr_score=0.0 if wr is None else max(0.0,min(100.0,(-wr-40.0)/40.0*100.0));ma_score=0.0 if ma100 is None or ma100<-5 else max(0.0,min(100.0,100.0-abs(ma100-2.0)*8.0));q_score=max(0.0,min(100.0,quality or 0.0));conf_score=confirmation_count/max(available_confirmations,1)*100.0;score=round(wr_score*.35+ma_score*.30+q_score*.20+conf_score*.15,2);as_of=str(market.get("as_of") or feature_as_of or (bars[-1].bar_date if bars else ""))[:10] or None;event_key=f"{MODEL_VERSION}:{s}:triggered:{as_of}" if state=="triggered" and as_of else None
    return {"symbol":s,"state":state,"raw_state":raw_state,"state_code":STATE_ORDER[state],"convergence_score":score,"williams_r":wr,"ma100_distance_pct":ma100,"ma200_distance_pct":ma200,"quality_score":quality,"quality_detail":quality_detail,"relative_volume":rv,"confirmations":confirmations,"confirmation_count":confirmation_count,"confirmation_available":available_confirmations,"confirmation_possible":len(confirmations),"confirmation_evidence_ready":evidence_ready,"price_context":ctx,"data_alignment":alignment,"missing_required_inputs":missing,"as_of":as_of,"retrieved_at":retrieved_at.isoformat() if hasattr(retrieved_at,"isoformat") else retrieved_at,"model_version":MODEL_VERSION,"model_config_hash":MODEL_CONFIG_HASH,"model_config":MODEL_CONFIG,"event_key":event_key,"alert_ready":state=="triggered" and alignment["ready_for_alert"],"interpretation":"Attractive stock nearing a technically favorable entry. Williams reset is primary, 100MA approach from above is secondary, the quality gate is independent of technical/momentum inputs, and confirmations distinguish stabilization from an accelerating decline."}
def evaluate_convergence(db:Session,symbol:str)->dict:
    s=symbol.strip().upper();market,retrieved_at=_latest_market(db,s);feature,feature_as_of=_latest_feature(db,s);bars=_bars(db,s);return evaluate_convergence_inputs(s,market,feature,bars,retrieved_at,feature_as_of)
