...
MODEL_VERSION="opportunity-convergence-v1.3"
...
def _quality(feature:dict):
    ...
    if valuation is not None:vals.append((valuation,.55,"valuation"))
    if sector is not None:vals.append((sector,.45,"sector_theme"))
    if not vals:return None
    weight=sum(w for _,w,_ in vals);score=round(sum(v*w for v,w,_ in vals)/weight,2)
    return score
