...
class HistoricalDailyBar(Base):
    ...
    __table_args__ = (UniqueConstraint("symbol", "bar_date", name="uq_historical_symbol_date"),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    symbol: Mapped[str] = mapped_column(String(20), index=True, nullable=False)
...
class SecondaryVerificationCache(Base):
    ...
    provider: Mapped[str] = mapped_column(String(64), nullable=False)
    payload: Mapped[dict] = mapped_column(JSON, nullable=False)
    retrieved_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), index=True, nullable=False)
...
class FlowEvent(Base):
    ...
    symbol: Mapped[str] = mapped_column(String(20), index=True, nullable=False)
    provider: Mapped[str] = mapped_column(String(64), nullable=False)
    outlier_score: Mapped[float] = mapped_column(Float, index=True, nullable=False, default=0.0)
    source_url: Mapped[str] = mapped_column(String(1024), nullable=False)
    payload: Mapped[dict] = mapped_column(JSON, nullable=False)
