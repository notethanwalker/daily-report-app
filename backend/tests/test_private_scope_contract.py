...
class PrivateScopeContract(unittest.TestCase):
    ...
    def test_opportunities_use_authenticated_tracked_scope_and_shared_market_cache(self):
        self.assertIn('@router.get("/tracked")', OPPORTUNITY_ROUTER)
        self.assertIn("UserWatchlistItem.user_email == user", OPPORTUNITY_ROUTER)
        self.assertIn("PortfolioHolding.user_email == user", OPPORTUNITY_ROUTER)
        self.assertIn("PortfolioDefinition.user_email == user", OPPORTUNITY_ROUTER)
        self.assertIn('@router.get("/market-scan")', OPPORTUNITY_ROUTER)
        self.assertIn("user: str = Depends(current_user)", OPPORTUNITY_ROUTER)
        self.assertIn('api("/api/v1/opportunities/tracked")', OPPORTUNITY_UI)
        self.assertIn('api("/api/v1/opportunities/market-scan?limit=200")', OPPORTUNITY_UI)
