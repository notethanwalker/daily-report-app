...
class RotationStateTests(unittest.TestCase):
    ...
    def test_observation_date_can_make_recent_retrieval_stale(self):
        ...
        row = SimpleNamespace(retrieved_at=datetime(2026, 9, 8, 19, tzinfo=timezone.utc), as_of="2026-09-01")
...
class FeatureVersionTests(unittest.TestCase):
    ...
    def test_pre_cutover_snapshot_remains_legacy(self):
        payload = presentation_payload({"buy_score": 60.0}, "2026-09-07")
        self.assertEqual(payload["model_version"], "legacy_unversioned")
        self.assertIsNone(payload["model_config_hash"])

    def test_post_cutover_snapshot_can_use_current_version(self):
        payload = presentation_payload({"buy_score": 60.0}, "2026-09-08")
        self.assertEqual(payload["model_version"], MODEL_VERSION)
