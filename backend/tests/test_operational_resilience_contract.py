...
class OperationalResilienceContract(unittest.TestCase):
    ...
    def test_smoke_identity_is_narrow_and_deployment_aware(self):
        ...
        self.assertIn("Resolve expected backend revision", SMOKE)
        self.assertIn("Wait for exact backend revision", SMOKE)
        self.assertIn("EXPECTED_BACKEND_SHA=$(git log -1 --format=%H -- backend)", SMOKE)
        self.assertIn('DEPLOYED_SHA" = "$EXPECTED_BACKEND_SHA', SMOKE)
